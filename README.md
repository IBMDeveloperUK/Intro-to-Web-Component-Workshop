...
4. Create a new directory in the `views` folder called `components` (so it's `views/components`). We're not going to put anything in it quite yet, but we need it to exist before we can run the server. 
